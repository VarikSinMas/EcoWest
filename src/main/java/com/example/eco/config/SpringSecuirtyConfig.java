...
public class SpringSecuirtyConfig extends WebSecurityConfigurerAdapter {
	...
	@Override
	protected void configure(HttpSecurity http) throws Exception { // ,"/points/","/addEvent"
		http.csrf().disable().authorizeRequests().antMatchers("/uploads/**","/points/","/authenticate", "/register","/getEvents/**").permitAll().anyRequest()
